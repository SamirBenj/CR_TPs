...
$docTypeDocCode = isset($_POST['docTypeDocCode']) ? $_POST['docTypeDocCode'] : '';
    ...
    if(isset($_POST['submit'])) {
        ...
        if((empty($docRef) || empty($docTitre) || empty($docAuteur) ||empty($docAnnee) ||empty($docEditeur) || empty($docGenre) || empty($docTypeDocCode ))) {
            ...
            $sql = "INSERT INTO DOCUMENT (docRef,docTitre,docAuteur,docAnnee,docEditeur,docGenre,docTypeDocCode) VALUES ('$docRef','$docTitre' ,'$docAuteur','$docAnnee', '$docEditeur','$docGenre','$docTypeDocCode')";

            if(!mysqli_query($con,$sql)) {
                echo '<b>Un document n a pas etait créer<b>';
            }else
                {
                    echo '<b>le document est creer<b>';
                }
